...
.NOTES
    ...
    Saved with a UTF-8 BOM on purpose: this file, and the two files it edits, all contain
    non-ASCII characters (em-dashes, section signs) in comments this script never touches.
    Windows PowerShell 5.1 does not reliably assume UTF-8 for a BOM-less script or a
    BOM-less Get-Content read  -  without the BOM here, and -Encoding UTF8 below, those bytes
    get silently reinterpreted under the console's codepage and the script (or the files it
    writes) end up corrupted. If you ever re-save this file, keep it UTF-8 with BOM.
#>
...
Write-Host '  3. Still open before the next phase (printing + PAN-based disposal): Q3 (transient print status), Q4 (ProductItem RowVersion), Q5 (retroactive backfill), Q7 (PAN-disposal scope), Q8 (Known-way through print?).'
